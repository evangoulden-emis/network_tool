...
def fqdn(_fqdn: str, nameserver: str = None):
    rdtypes = ['A', 'AAAA', 'CNAME', 'MX', 'NS', 'SOA', 'PTR', 'SRV', 'TXT', 'CAA', 'DS', 'DNSKEY', 'RRSIG', 'NSEC', 'NSEC3', 'NSEC3PARAM']
    console = Console()
    ...
    try:
        ...
        if nameserver:
            ...
            console.print(f"[bold]FQDN:[/bold] {_fqdn}, [bold]Nameserver:[/bold] {system_nameservers[0] if system_nameservers else 'unknown'}")

        # Create a table
        table = Table(title=f"DNS Records for {_fqdn} using nameserver: {nameserver if nameserver else system_nameservers[0]}", show_lines=True)
